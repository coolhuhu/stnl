...
void SocketUtil::bindAddress(int socketFd, sockaddr *addr)
{
...
{
    if (ipv6_) {
        return static_cast<struct sockaddr*>(&addr6_);
    }
    else {
...
int Socket::accept(SockAddr& addr)
{
    ...
    int connect_fd = SocketUtil::acceptSocket(socketFd_, static_cast<struct sockaddr*>(&addr6));
    if (connect_fd >= 0) {
